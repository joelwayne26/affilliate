  ...
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 hover:-translate-y-1 flex flex-col">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        {/* Product Name */}
        <h3 className="font-bold text-[#1a2744] text-base mb-2 font-[family-name:var(--font-montserrat)] group-hover:text-[#3b82f6] transition-colors">
          {product.name}
        </h3>
    ...
    <AnimatedSection animation={index % 2 === 0 ? "fade-left" : "fade-right"} delay={index * 100}>
      <section id={niche.id} className="py-12 scroll-mt-20">
        ...
        >
          ...
          <div>
            <span className="text-xs font-bold uppercase tracking-wide" style={{ color: niche.color }}>
              Pinterest Tip
            </span>
            <p className="text-sm text-[#1a2744]/70 mt-0.5">{niche.pinterestTip}</p>
          </div>
        </div>
...
const faqs = [
  {
    ...
    answer:
      "Each product card shows 'Best for' to help you match tools to your needs. If you're just starting, begin with free trials — most of these tools offer one. I only recommend products I've researched thoroughly or use myself.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Start by choosing a niche you're passionate about. Sign up for the affiliate programs listed here (Amazon, ShareASale, ClickBank, Shopify). Create content — blog posts, Pinterest pins, social media — that genuinely helps people. Insert your affiliate links naturally. Be patient and consistent.",
  },
  {
...
export default function Home() {
  ...
  const [activeProgramFilter, setActiveProgramFilter] = useState<Program | "all">("all");
  ...
  const programCounts: Record<string, number> = { all: 0 };
  niches.forEach((n) =>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-10">
            <div className="max-w-3xl">
              ...
              <p className="text-lg sm:text-xl text-blue-100/90 mb-8 leading-relaxed max-w-2xl animate-fade-in-up delay-400">
                ...
                Every product tagged by affiliate program so you know exactly where you&apos;re buying.
              </p>
                ...
                return (
                  <AnimatedSection key={key} animation="scale" delay={i * 100}>
                    ...
                    >
                      ...
                      >
                        {key === "amazon" && <span className="text-white text-lg">A</span>}
                        {key === "shareasale" && <span className="text-white text-lg">S</span>}
                        {key === "clickbank" && <span className="text-white text-lg">C</span>}
                        {key === "shopify" && <span className="text-white text-lg">S</span>}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold" style={{ color: prog.color }}>
                          {programCounts[key] || 0} products
                        </span>
                        <a href={`#${key}-products`} className="text-xs font-medium hover:underline" style={{ color: prog.color }}>
                          View all →
                        </a>
                      </div>
                    </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                },
                {
                  ...
                  desc: "Every product shows which affiliate program it's from — Amazon, ShareASale, ClickBank, or Shopify. Full transparency.",
                },
                {
                  icon: "💰",
                  title: "Know the Commission",
                  desc: "Commission rates are displayed upfront. From 4.5% on Amazon to 75% on ClickBank, you know exactly what's earned.",
                },
                {
                  icon: "✅",
      ...
      <footer className="bg-[#1a2744] text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-blue-200/60">
                {niches.map((n) => (
                  <a key={n.id} href={`#${n.id}`} className="block hover:text-white transition-colors">
                    {n.icon} {n.title}
                  </a>
                ))}
              </div>
          ...
          <div className="mt-10 pt-6 border-t border-white/10 text-center text-xs text-blue-200/40">
            © {new Date().getFullYear()} Joel Affiliates. All rights reserved. |{" "}
